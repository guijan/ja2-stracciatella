...
static void HandleGameSaved()
{
	...
	for (auto s : GCM->getMapSecrets())
	{
		if (!s->isSAMSite && (++i > 1))
		{
			...
			ST::string sector = SECTOR_SHORT_STRING(s->sectorID);
			secretsFound[sector] = IsSecretFoundAt(s->sectorID);
		}
	}
	if (!secretsFound.empty())
	{
		STLOGD("Saving extra map secrets ({} items)", secretsFound.size());
...
static void HandleGameLoaded()
{
	if (g_gameStates.HasKey(MAPSECRETS_GAMESTATE_KEY))
	{
		...
		{
			UINT8 sector = SECTOR_FROM_SECTOR_SHORT_STRING(secretStatus.first.c_str());
			SetSectorSecretAsFound(sector, secretStatus.second);
		}
	}
}